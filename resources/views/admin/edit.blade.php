<form action="{{ route('admin.update',$review->id) }}" method="POST" class="py-3" >
    @csrf
    @method('PUT')
    ...
    <div class="flex flex-wrap -mx-3 mb-1" >
        ...
        <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0" >
            <label for="name_first" class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" >First
                Name</label >
            <input type="text" id="name_first" name="name_first" value="{{ $review->name_first }}"
        ...
        <div class="w-full md:w-1/2 px-3" >
            <label for="name_last" class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" >Last
                Name</label >
            <input type="text" id="name_last" name="name_last" value="{{ $review->name_last }}"
    ...
    <div class="flex flex-wrap -mx-3 mb-1" >
        ...
        <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0" >
            <div class="range-slider" >
                ...
                <input id="rate" name="rate" class="range-slider__range" type="range" value="{{ $review->rate }}"
                       min="1.0" max="5.0"
                       step="0.5" >
                <span id="rate_value" class="range-slider__value" >{{ $review->rate }}</span >
            </div >
        </div >
